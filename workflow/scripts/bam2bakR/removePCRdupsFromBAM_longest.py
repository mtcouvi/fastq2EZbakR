...
Remove reads in bam file resulting from PCR duplicates and keep the longest
"""
import sys, pysam
...
for read in iBAM.fetch(until_eof=True):
    ...
    key = f"{chrom}_{std}_{mb}"
    try:
        value = read.get_tag("NM")
    except KeyError:
        continue

#     if key not in MB or (value < MB[key][0] and length > MB[key][2]):
#         MB[key] = (value, read, length)
    if key not in MB or length > MB[key][0]:
        MB[key] = (length,read)
